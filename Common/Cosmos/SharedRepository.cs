...
{
    public class SharedRepository<T> : IRepository<T>
                where T : class, IEntity
    {

        private string _databaseName = "TrailCamDev";

        public SharedRepository(CosmosClient client)
        {
            Container = client.GetDatabase(_databaseName).GetContainer(typeof(T).Name);
        }
